...
@Component({
  selector: 'app-pending-ra',
  templateUrl: './pending-ra.component.html',
  styleUrls: ['./pending-ra.component.scss']
})
export class PendingRAComponent {
  ...
  gotoEventDashboard(eventDetail: any) {
    ...
    this.router.navigate(['/Event/EventDashboard'], {
      state: { EventId: eventDetail.evenT_ID,eventDetail: eventDetail, approvedRejectButtonCondition:true },
    });
  }
  ...
  approveRejectButton(item:any,isApproved:boolean){
...
this.commonService.showAlertForWarning('Are You Sure','You want to approve').then((res) => {
  ...
  CallApproveApi(){
    ...
    this.approveService.ApproveReverseAuction(this.itemSelected.evenT_ID)
    .subscribe({
      next: (result: any) => {
