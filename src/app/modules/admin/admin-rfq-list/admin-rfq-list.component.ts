...
import { ReasonModalComponent } from '../../event/components/reason-modal/reason-modal.component';


@Component({
  selector: 'app-admin-rfq-list',
  templateUrl: './admin-rfq-list.component.html',
  styleUrls: ['./admin-rfq-list.component.scss'],
})
export class AdminRfqListComponent {
  ...
  public ngOnInit() {
    ...
    // this.commonService.isReadOnlyRfqcsView$.next()
    ...
    this.eventService
      ...
      .subscribe({
        next: (result: IDefaultResponseDto<IRfqcsListDataDto[]>) => {
          if (result.success) {
            let resultFilterData = result.data
              .map((o: any) => {
                o.evenT_NO = parseInt(o.evenT_NO);
                o.createD_DATE = o.createD_DATE
                  ? new Date(
                    new Date((o.createD_DATE as any).split('T')[0]).setHours(
                      0,
                      0,
                      0
                    )
                  ...
                  : null;
                o['closinG_TIME_Format'] = o.closinG_TIME
                  ? new Date(
                    new Date((o.closinG_TIME as any).split('T')[0]).setHours(
                      0,
                      0,
                      0
                    )
              ...
              .filter((val: any) => val.evenT_STATUS != 'Deleted');
            this.loading = false;
  ...
  gotoEventDashboard(eventDetail: any) {
    ...
    this.router.navigate(['/Event/EventDashboard'], {
      state: { EventId: eventDetail.evenT_ID, isViewOnly: true },
    });
  }
  ...
  openReasonModal(item: IRfqcsListDataDto, index: number) {
    this.commonService.clearToaster();
    const modal: NgbModalRef = this.modalService.open(ReasonModalComponent, {
      centered: true,
    });
