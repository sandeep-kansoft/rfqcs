...
@Component({
  ...
  styleUrls: ['./reverse-auction-list.component.scss']
})
export class ReverseAuctionListComponent {
  ...
  getReverseAuctionList() {
    ...
    this.loading = true;
    this.eventService
      .getReverseAuctionListApi(payload)
  ...
  isReactiveButton(item:any,index:number){
    this.commonService
      .showAlertForWarning(
        'Reactive',
      ...
      .then((result) => {
        if (result) {
          this.reactiveApi(item, index);
        }
      });
  }
reactiveApi(item:any,index:number){
  this.eventService.SendReactiveApi(item.evenT_ID).subscribe({
    next: (result: any) => {
      if (result.success) {
        this.commonService.showToaster('Event Reactivated Successfully', true);
       this.getReverseAuctionList();
       this.cdr.detectChanges();
      } else {
        this.commonService.showToaster(result.errorDetail, false);
